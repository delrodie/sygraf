...
use AppBundle\Utils\Gestiondistrict;
...
class DistrictController extends Controller
{
     ...
     * @Method({"GET", "POST"})
     */
    public function newAction(Request $request, Gestiondistrict $gestiondistrict)
    {
        ...
        if ($form->isSubmitted() && $form->isValid()) {
            $em = $this->getDoctrine()->getManager();
            if (!$gestiondistrict->duplicate($district->getLibelle())){
                $em->persist($district);
                $em->flush();
                $this->addFlash('notice', 'Le district '.$district->getLibelle().' a été ajouté avec succès!');
            }else{
                $this->addFlash('error', "Echèc d'enregistrement !!! Car le district '.$district->getLibelle().' existe déjà.!");
            }

            return $this->redirectToRoute('district_new');
        }
        $em = $this->getDoctrine()->getManager();
        $districts = $em->getRepository('AppBundle:District')->findAll();
